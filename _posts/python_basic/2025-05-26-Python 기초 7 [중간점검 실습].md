...
# 프로세스 플로우 차트 흐름 구현 : 키오스크
# 1. 어떤 데이터 타입으로 메뉴에 대한 저장을 하는지
# 2. 처음으로/뒤로가기 등 흐름을 구현하는 중첩 구조: break/continue와 반복문
# 3. 반복되는 처리 구간에 대한 기능단위(함수화)
# 4. 시간체트는 메뉴 카테고리 진입 시점에서 datetime 활용
# 5. 가격은 드라이브에 문서 참조
